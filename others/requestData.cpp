...
priority_queue<Timer*, deque<Timer*>, TimerCmp> TimerQueue;
...
void requestData::addTimer(Timer *mtimer) {
    if (timer == NULL)
        timer = mtimer;
}
...
void requestData::handleRequest() {
    ...
    Timer *mtimer = new Timer(this, 500);
    timer = mtimer;
...
int requestData::analysisRequest() {
    if (method == METHOD_POST) {
        ...
        // test char*
        const char *send_content = "I have receiced this.";
...
Timer::Timer(requestData *_request_data, int timeout): deleted(false), request_data(_request_data) {
    //cout << "Timer()" << endl;
    struct timeval now;
